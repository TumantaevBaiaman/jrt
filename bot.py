...
from config import TOKEN_BOT  # импортируем API-токен нашего бота из конфигурационного файла

storage = MemoryStorage()  # создаем объект MemoryStorage для хранения состояний пользователей

bot = Bot(token=TOKEN_BOT)  # создаем объект Bot с указанным API-токеном
